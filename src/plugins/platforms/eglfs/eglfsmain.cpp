...
{
    if (system.compare(QLatin1String("aurora-eglfs-next"), Qt::CaseInsensitive) == 0) {
        ...
        for (const auto &param : qAsConst(paramList)) {
            ...
            if (paramArgs.length() == 2) {
                if (paramArgs[0].trimmed().toLower() == QStringLiteral("disable-input-handlers"))
                    options.disableInputHandlers =
                            QVariant::fromValue(paramArgs[1].trimmed()).toBool();
            }
        }
