...
{
    m_window.reset(auroraDeviceIntegration()->createWindow(screen()->auroraOutput(), window));
}
...
void EglFSWindow::destroy()
{
    ...
    if (m_eglWindow) {
        auroraDeviceIntegration()->destroyNativeWindow(m_eglWindow);
        m_eglWindow = 0;
    }
...
bool EglFSWindow::createSurface()
{
    ...
    QSurfaceFormat platformFormat =
            auroraDeviceIntegration()->surfaceFormatFor(window()->requestedFormat());

    EglFSConfigChooser chooser(eglDisplay);
    chooser.setSurfaceType(auroraDeviceIntegration()->surfaceType());
    ...
    m_eglWindow =
            auroraDeviceIntegration()->createNativeWindow(m_window.data(), surfaceSize, m_format);
    if (!m_eglWindow) {
